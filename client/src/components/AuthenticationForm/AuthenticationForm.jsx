...
let AuthenticationForm = props => {
  ...
  const { authType } = globalState[0];
  const globalContextSetter = globalState[1];
  ...
  const handleFormSubmit = event => {
    ...
    if (formType === 'signup') {
      ...
      if (username.length !== 0 && password.length !== 0) {
        const result = submitHandler({ username, password });
        setSubmitResult(result);
        setSubmitted(true);
      }
    }
  ...
  const validate = (field, value) => {
    ...
    switch (field) {
      ...
      case 'confirmPassword':
        error =
          validatePassword(value) && password === value ? 'valid' : 'invalid';
  ...
  return (
    <div className={clsx(classes.auth, 'animated', 'fadeIn')}>
        ...
        >
          ...
          <FormControl>
            <InputLabel htmlFor='username'>
              Username {formType === 'signup' && '(optional)'}
            </InputLabel>
            <Input
              id='username'
              type='text'
